...
function calcProgression(user) {
  ...
  const weeklyInc =
    user.goal === "bulk"
      ...
      : 0;
  return { weeklyIncrease: weeklyInc };
}

function generatePlan(user) {
  // 1. filter safe exercises
  const safe = exports.exerciseDB.filter(
    (ex) =>
  ...
  // 3. build 26-week cycle
  const { weeklyIncrease } = calcProgression(user);
    ...
    const days = Array.from({ length: daysPerWeek }, (_, d) => {
      ...
      const exercises = pool
        ...
        .map((ex) => ({
          ...
          load: user.goal !== "weightLoss" ? `${(1 + weeklyIncrease * 0.01 * w).toFixed(2)}×` : "RPE 7",
        }));
